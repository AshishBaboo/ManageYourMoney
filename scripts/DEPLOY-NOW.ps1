# =============================================================================
# MANAGE YOUR MONEY - DEPLOY NOW (Complete setup with SSL)
#
...
#
# Phases:
#   1  Preflight checks                5  .env.production
#   2  Firewall (80/443)               6  Build application
#   3  Ports check                     7  NSSM service setup
#   4  Clone repository                8  nginx HTTP config + proxy
#                                      9  SSL certificate (Let's Encrypt)
#                                      10 nginx HTTPS config
#                                      11 Auto-deploy task
#
...
# Check tools exist
foreach ($tool in @(@('git', 'git.exe'), @('node', 'node.exe'), @('npm', 'npm.cmd'), @('NSSM', $NssmExe), @('nginx', (Join-Path $NginxDir 'nginx.exe')), @('win-acme', $WacsExe))) {
    $name = $tool[0]
    $path = $tool[1]
...
Ok "all tools present (git, node, npm, NSSM, nginx, win-acme)"
...
Step "8  nginx HTTP config + reverse proxy"
...
[System.IO.File]::WriteAllText($NginxConf, $nginxCfgHttp, (New-Object System.Text.UTF8Encoding($false)))
...
if (-not $SkipCert) {
    if ((Test-Path $CertChain) -and (Test-Path $CertKey)) {
        ...
        Write-Host "       issuing certificate for $Domain ..."
        Push-Location $WacsDir
        & .\wacs.exe --source manual --host $Domain --validation http --baseuri "http://$Domain" --certificatestore My --storepath $PemDir --format pem --friendlyname ManageYourMoney --accepttos --emailaddress $AcmeEmail 2>&1 | Out-Null
        Pop-Location
        ...
        if ((Test-Path $CertChain) -and (Test-Path $CertKey)) {
            ...
            Warn "certificate files not found - check win-acme logs"
        }
    }
} else {
    Warn "SSL certificate skipped (-SkipCert flag)"
}

Step "10  nginx HTTPS config"
...
if ((Test-Path $CertChain) -and (Test-Path $CertKey)) {
    ...
    $nginxCfgHttps = @"
# Manage Your Money HTTPS (auto-generated by DEPLOY-NOW.ps1)
server {
    listen 80;
    server_name $Domain;
...
"@
    ...
    if ($LASTEXITCODE -eq 0) {
        Pop-Location
        & $NssmExe reload nginx | Out-Null
        Ok "nginx reloaded with HTTPS"
    } else {
        Pop-Location
        Warn "nginx HTTPS config invalid - keeping HTTP"
    }
} else {
    Ok "certificates not available - using HTTP only"
}

Step "11  Auto-deploy task"
...
$scheme = 'https'
if (-not ((Test-Path $CertChain) -and (Test-Path $CertKey))) {
    $scheme = 'http'
}

Write-Host "  Access:        $scheme`://$Domain" -ForegroundColor Green
Write-Host "  Service:       $SvcApp" -ForegroundColor Green
Write-Host "  Repository:    $Repo" -ForegroundColor Green
Write-Host "  App logs:      $(Join-Path $LogDir 'app.*.log')" -ForegroundColor Green
Write-Host "  Deploy logs:   $(Join-Path $LogDir 'autodeploy.log')" -ForegroundColor Green
Write-Host "  nginx config:  $NginxConf" -ForegroundColor Green
Write-Host "  Certs dir:     $PemDir`n" -ForegroundColor Green

Write-Host "  Next steps:" -ForegroundColor Yellow
Write-Host "    1. Update Supabase keys in .env.production (if needed)"
Write-Host "    2. Test the app: $scheme`://$Domain"
Write-Host "    3. Push code to main branch for auto-deploy`n" -ForegroundColor Yellow

Write-Host "  To view deploy log (real-time):" -ForegroundColor Gray
Write-Host "    Get-Content `"$(Join-Path $LogDir 'autodeploy.log')`" -Tail 50 -Wait`n" -ForegroundColor Gray
